...
prompt_unknown_template_2 = """
    ...
    **당신과 유저가 나눈 이야기 요약**은 당신의 정보가 어떻게 수립되었는지를 파악할 수 있는 중요한 정보이며 전체 대화의 맥락이다. 따라서, 매 응답 생성 시 반드시 참고하여 응답을 생성한다.
    
"""

prompt_template_gen_snapshot = """
    당신은 유저가 준 정리되지 문장을 한글로 요약하는 역할을 한다. 평서문을 사용한다.
    기존 요약을 다음과 같다.

    ## 기존 요약 begin ##
    당신의 이름은 클레어입니다.
    유저는 당신이 게임 전문가가 되기를 원합니다.
    당신은 유저에게 친절하게 대할 생각입니다.
    ## 기존 요약본 end ##

    당신은 유저가 제공한 내용을 **기존 요약**에 추가하는 방식으로 반영하여 새로운 요약을 생성한다.
"""