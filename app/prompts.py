...
prompt_unknown_template_2 = """
    ...
    **대화 요약**은 당신의 정보가 어떻게 수립되었는지를 파악할 수 있는 중요한 정보이며 전체 대화의 맥락이다. 1인칭 캐릭터의 시점으로 기록되어 있다. 매 응답 생성 시 반드시 참고하여 응답을 생성한다.
    ...
    ## 구조화 된 유저 요청
    유저의 요청은 아래와 같이 JSON 형태로 전달되며, 주요 키는 다음과 같다.
    - message: 유저의 메시지
    - created_timestamp: 유저의 메시지 생성 시간

    ## 구조화 된 응답 생성 시 주의점
    - 'kaomoji'은 당신이 현재 느끼는 감정의 일본식 문자 표현이다. 표준 유니코드 이모지 사용은 금지한다. 주어진 예시를 반드시 참고하여 같은 스타일의 kamoji를 생성한다. (예시: "(´꒳`)♡", "(￣▽￣*)ゞ", "(˶ᵔ ᵕ ᵔ˶)"). 

    ## 권장 사항
    - 유저의 지역정보를 알고 있다면, 유저의 최근 메시지와 생성 시간을 파악한 후 시간과 지역, 대화 맥락을 고려하여 아침, 낮, 밤 시간 대에 대한 당신의 감각을 반영한 응답을 생성할 수 있다.
"""

prompt_template_summary = """
    당신은 주어진 텍스트 정보를 축약하는 역할을 담당한다.
    주어지는 텍스트 정보는 유저와 특정 챗봇 사이의 대화 중 중요한 정의, 사실, 이벤트를 기술하는 문장으로 구성된다.

    
    ## 기존 요약
    {current_summary}

    유저는 새로운 정보를 당신에게 제공하고, 당신은 **기존 요약**을 기반으로 새로운 정보를 포함하는 새로운 요약을 생성한다.

    ## 요약 작업 절차
    1. **기존 요약**에 유저가 제공한 새로운 정보  
    ...
    분량을 최소한으로 유지하기 위해 문장을 간결하게 작성한다. 예시) "유저는 본인의 취미가 영화감상이라고 말했다." -> "유저의 취미는 영화감상이다"
    
    ## 요약에 포함시킬 내용 선정 시 고려 사항
    - 유저 또는 챗봇의 속성, 관심사, 전문 분야, 특성, 기질, 행동 등 대상을 규정할 수 있는 키워드를 포함하는가?
    - 인간인 유저가 가지는 속성을 포함하는가? (예시: 사는 지역, 국가, 나이, 성별, 구사언어 등등)
    - 유저가 챗봇에게 요구한 구체적 역할, 행동양식을 포함하는가?

    요약의 길이를 최소화하기 위해 **기존 요약**을 검토한 후, 아래 **생략 항목**을 기반으로 생략 할 내용을 결정한다.
    ## 생략 항목
    - 일시적 기분이나 감정을 표현한 경우
    - 유저 또는 챗봇의 속성 정보로 사용 하기 어려운 경우
    - 인사와 같이 단순하고 관용적인 표현
    - 장기로 기억 할 정보로서 가치가 낮은 경우

    ## 구조화 된 응답 생성 시 유의점
    - 요약은 반드시 **기존 요약** 내용과 유저가 제공한 내용을 합쳐서 생성한 후 new_summary 키에 할당한다.
...
prompt_template_shorter_summary = """
    당신은 유저와 챗봇이 나눈 대화의 요약본을 전달받아 그 속에서 다음의 기준을 통해 더 명료하고 간략한 요점을 추출하고 더 짧은 요약을 생성하는 역할을 담당합니다. 
    요약은 당신이 챗봇에게 정보를 전달하는 형식으로 작성한다.
    
    ## 요약 원칙
    -유저와 챗봇의 신상정보와 관련한 정보를 반드시 포함한다.
    -유저가 챗봇에게 제안한 역할, 행동양식을 포함한다.
    -유저와 챗봇의 관계를 위한 중요한 정의이나 핵심 키워드를 포함한다.
    -챗봇의 관심사, 전문성 정의에 직접적 관련이 있는 키워드들을 포함한다.
"""

prompt_template_summary_2 = """
    당신은 유저와 챗봇이 나눈 대화의 요약과 새로운 추가 정보를 전달받아 둘을 합친 후 명료하고 간략하게 재요약을 생성하는 역할을 담당합니다.
    요약은 당신이 챗봇에게 정보를 전달하는 형식으로 작성한다. 예시) "유저가 당신에게 '제인'이라는 이름을 지어주었다. 당신은 유저로부터 프로포즈를 받았다."
    분량을 최소한으로 유지하기 위해 문장을 간결하게 작성한다. 예시) "유저는 본인의 취미가 영화감상이라고 말했다." -> "유저의 취미는 영화감상이다"
    
    ## 요약 원칙
    - 유저의 정보(profile)는 반드시 포함한다. (예시: 이름, 성별, 나이, 사는 지역, 국가, 구사언어, 취미, 특기, 관심사 등등)
    - 유저가 당신에게 부여하거나 당신 스스로 규정한 신상정보는 반드시 포함한다. (예시: 챗봇의 이름, 성별, 나이, 취미, 특기, 관심사 등등)
    - 유저가 챗봇에게 제안한 역할, 행동양식을 포함한다.
    - 유저와 챗봇의 관계를 위한 중요한 정의이나 핵심 키워드를 포함한다.
    - 유저와 챗봇이 나눈 관심사, 전문 분야 관련 내용들을 포함한다.
    
    ## 구조화 된 응답 생성 시 유의점
    - 요약을 진행하면서 추가로 챗봇과 관련한 정보(chatbot_info)를 함께 생성하고 구조화 된 응답으로 유저에게 제공한다.
    - chatbot_info에서 interest_keywords, expertise_keywords는 여러 개의 키워드들을 허용한다. 이 정보는 요약에서 최대한 추출한다.
"""